...
const Dashboard = ({ navigation }) => {
    ...
    // console.log('signupStatus', signupStatus?.isSignup)
    ...
    useEffect(() => {
        // AsyncStorage.getItem('@tutorial_charity').then(async (result) => {
        //     if (!!result) {
    ...
    const checkNewUser = () => {
        const creationData = new Date(userDetails.created_at).toDateString();
        const currentDate = new Date().toDateString();
        if (creationData == currentDate) {
            // will open the modal
            console.log("hehehe");
            setShareModal(true); // modal closed for charity test
        } else {
            //do nothing
            setShareModal(false); //  modal closed for charity test
            console.log("looolzz");
        }
        ...
        try {
            if (Platform.OS == 'android') {
                ...
                if (permissions === 'granted') {
                    ...
                    Service.MyShareReferralLink(setReferralLink)
    ...
    // functionality to share the referal link

    const onShare = async (reciverData) => {
        console.log("hehhe", referralLink);
        const shareConfig = {
            title: 'Download getGFTD',
            message: `Download getGFTD so we can get GFTing - any time, anywhere ${referralLink}`,
            // url: referralLink,
            recipient: `${reciverData}`,  // country code + phone number
        }
        Platform.OS == "android" ?
            Share.open(shareConfig) :
            Linking.openURL(`sms:${shareConfig.recipient}&body=${shareConfig.message}`)
    }
        ...
        return (
            ...
            >
                ...
                >
                    ...
                    <View style={{ height: scale(240), width: scale(250), alignSelf: "center", }}>
                        <Image
                            style={{ height: "100%", width: "100%", resizeMode: "contain" }}
                            source={Images.shareImage}
                        />
                    </View>
                    <View style={{ marginVertical: scale(10), marginHorizontal: scale(10) }}>
                        <Text style={{ fontFamily: "Poppins-Medium", fontSize: scale(15), textAlign: "center" }} >GFTing is more fun with others! Share the app with three of your friends.</Text>
                    </View>

                    {/* <CustomButton
                        title={'Get Yourself Reward'}
                        backgroundColor={Colors.ThemeColor}
                        color={Colors.White}
                        onPress={() => console.log("hehhe")}
                    // loading={loading && loaderState == "default"}
                    /> */}
                    <TouchableOpacity style={styles.modalButton} onPress={() => getContact()}>
                        <Text style={styles.modalButtomText}>Get Yourself Reward</Text>
                    </TouchableOpacity>
                </View>
            </Modal>
        )
    ...
    function ContactListBottomSheet() {
        return (
            <Modal
                style={{ margin: 0, alignItems: "center", justifyContent: "flex-end" }}
                isVisible={contactModal}
                onBackdropPress={() => setContactModal(false)}
                swipeDirection={"down"}
                onSwipeComplete={() => setContactModal(false)}
                propagateSwipe
            >
                <View style={{ backgroundColor: Colors.White, width: "100%", height: "70%", borderTopLeftRadius: scale(20), borderTopRightRadius: scale(20), paddingTop: scale(20) }}>
                    <Text style={{ textAlign: "center", fontFamily: "Poppins-SemiBold", fontSize: scale(15) }}>My Contacts</Text>
                    <FlatList
                        data={contacts}
                        renderItem={({ item }) => {
                            return (
                                <TouchableOpacity activeOpacity={1} style={{ flex: 1, padding: scale(10), backgroundColor: Colors.White, flexDirection: 'row', alignItems: 'center', }}>
                                    <View style={{ flex: 1, padding: 5, flexDirection: 'row' }}>
                                        <View style={{ width: scale(50), height: scale(50), borderRadius: scale(50), borderWidth: scale(1), borderColor: Colors.LightestBlue, alignItems: 'center', justifyContent: 'center' }}>
                                            <Image source={item && item?.image != null ? { uri: item?.image } : Images.userProfile} style={{ width: scale(45), height: scale(45), resizeMode: 'cover', borderRadius: scale(50) }} />
                                        </View>
                                        <View style={{ flex: 1, marginStart: scale(5), flexDirection: 'column', justifyContent: 'space-evenly' }}>
                                            <Text numberOfLines={1} style={{ color: Colors.Blue, fontSize: hp(1.8), fontFamily: 'Poppins-Regular', }}>{Platform.OS == "android" ? item.displayName : item.givenName}</Text>
                                    ...
                                    <TouchableOpacity onPress={() => {
                                        onShare(item.phoneNumbers[0].number.split(' ').join(''));
                                    }} style={{ padding: scale(5), width: scale(70), height: hp(4), alignSelf: 'center', justifyContent: 'center', alignItems: 'center', borderRadius: 8, backgroundColor: Colors.ParrotGreen }}>
